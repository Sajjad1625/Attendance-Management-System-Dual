...
# t1 = threading.Thread(target = speak_greetings.main)
# t1.start()
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        if username == "niltech" and password == "Niltech@12345":
            ...
            return render_template('index.html', user=(session['user']), late_time=(late_hour, late_minute))
        else:
            try:
...
def attendance():
    if 'user' in session.keys():
        conn = sqlite3.connect('./files/data.db')
        cursor = conn.execute("SELECT NAME, ID, BRANCH, DAY, MONTH, YEAR, LOGIN_TIME, LOGOUT_TIME from ATTENDANCE")
        attendance_list = []
        for row in cursor:
...
def download_all():
    if 'user' in session.keys():
        ...
        for i in attendance_list:
            ...
            logout_time.append(i[7])

        data = pd.DataFrame({'Name': name, 'ID': rollno, 'Branch': branch, 'Date': date, 'Login_Time': login_time, "Logout_Time": logout_time})
...
def download_month():
    if 'user' in session.keys():
        ...
        conn = sqlite3.connect('./files/data.db')
        cursor = conn.execute(f"SELECT NAME, ID, BRANCH, DAY, MONTH, YEAR, LOGIN_TIME, LOGOUT_TIME from ATTENDANCE where MONTH = '{cur_m}' AND YEAR = '{cur_y}'")
        attendance_list = []
        for row in cursor:
            attendance_list.append(row)
        conn.close()
...
def download_specific(rollno):
    if 'user' in session.keys():
        ...
        conn = sqlite3.connect('./files/data.db')
        cursor = conn.execute(f"SELECT NAME, ID, BRANCH, DAY, MONTH, YEAR, LOGIN_TIME, LOGOUT_TIME from ATTENDANCE where ID = '{rollno}' ")
        attendance_list = []
        for row in cursor:
            attendance_list.append(row)
        conn.close()
        ...
        data = pd.DataFrame({'Name': name, 'ID': rollnox, 'Branch': branch, 'Date': date, 'Login_Time': login_time, 'Logout_Time':logout_time})
...
def profile(ID):
    if 'user' in session.keys():
        ...
        cursor = conn.execute(f"SELECT NAME, ID, BRANCH, DAY, MONTH, YEAR, LOGIN_TIME, LOGOUT_TIME from ATTENDANCE where ID = '{ID}'")

        attendance_list = []
        for row in cursor:
            attendance_list.append([row[3],row[4],row[5],row[6],row[7]])
        conn.close()
...
def delete_user(ID):
    if 'user' in session.keys():
        ...
        try:
            ...
            f1 = open("f./files/ace_data_change.txt", 'w')
            f1.write("1")
            f1.close()
            try:
                os.remove(AUDIO_FOLDER+f"{ID}.wav")
...
@app.route('/reboot/<val>')
def system_boot(val):
    if 'user' in session.keys():
        if int(val) == 1:
            os.system('shutdown /r /t 0') #Restart in 0 seconds
        elif int(val) == 2:
            os.system('shutdown /s /t 0') #Shutdown in 0 Seconds
        else:
            return redirect(url_for('login_page'))
...
def generate2(): #Generate Video Frame
    image_resolution2 = get_image_resolution2()
    while True:
        try:
            # shared_image2 = shared_memory.SharedMemory(name='shared_raw_image2')
            # processed_image2 = np.ndarray((image_resolution2[1], image_resolution2[0], 3), dtype=np.uint8, buffer=shared_image2.buf)

            shared_image2 = shared_memory.SharedMemory(name='shared_processed_image2')
            processed_image2 = np.ndarray((image_resolution2[1], image_resolution2[0], 3), dtype=np.uint8, buffer=shared_image2.buf)

            while True:
                try:
                    flag, encoded_image = cv2.imencode('.png', processed_image2)
                except Exception as e:
                    print(e)
            ...
            print("Error in Generate 2:", e)
            time.sleep(0.5)


@app.route('/video_feed2')
def video_feed2(): # Show Video Feed
    if 'user' in session.keys():
...
def send_mail():
    if 'user' in session.keys():
        if request.method == 'POST':
            ...
            tomail = request.form['parentmail']
            subject = request.form['mailsubject']
            message = request.form['mailbody']

            sendmail(None, subject, message, tomail)

            return html_return("Mail Sent Initiated to: "+str(tomail), redirect_to="/all_users/", delay = 3)
        return html_return("Mail Sent wasn't Initiated to: "+str(tomail), redirect_to="/all_users/", delay = 3)
    else:
        return redirect(url_for('login_page'))
    

@app.route('/sendmail_bulk/', methods=['get', 'post'])
def send_mail_bulk():
    if 'user' in session.keys():
        if request.method == 'POST':

            print("Sending Bulk Mail")

            all_mails = []
            all_mails_str = "<br>"
            count = 0
            conn = sqlite3.connect('./files/data.db')
            cursor = conn.execute(f"SELECT PARENT_NAME, PARENT_MAIL from USER")

            for row in cursor:
                count += 1
                all_mails.append(row[1])

                all_mails_str += str(count)+". " + row[0] + " - "
                all_mails_str += row[1]
                all_mails_str += "<br>"
            
            for tomail in all_mails:
                subject = request.form['mailsubject']
                message = request.form['mailbody']

                sendmail(None, subject, message, tomail)

            return html_return("Mail Sent Initiated for all Parents: "+ all_mails_str, redirect_to="/", delay = 3)
        return html_return("Mail Sent wasn't Initiated for all Parents: "+ all_mails_str, redirect_to="/", delay = 3)
    else:
        return redirect(url_for('login_page'))
